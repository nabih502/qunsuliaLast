...
const PowerOfAttorneyForm = () => {
  ...
  const handleSubmit = async (formData) => {
    const referenceNumber = `POA-${new Date().getFullYear()}${(new Date().getMonth() + 1).toString().padStart(2, '0')}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`;

    console.log('Power of Attorney Application:', { ...formData, referenceNumber });

    navigate('/success', {
      state: {
        referenceNumber,
        serviceTitle: 'التوكيلات'
      }
    });
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1 text-center px-4">
              <h1 className="text-2xl sm:text-3xl font-bold">التوكيلات</h1>
            </div>
