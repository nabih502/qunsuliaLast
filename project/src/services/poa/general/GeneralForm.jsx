...
const GeneralForm = () => {
  ...
  const handleSubmit = async (formData) => {
    ...
    // Simulate API call
    setTimeout(() => {
      const referenceNumber = `GEN-${new Date().getFullYear()}${(new Date().getMonth() + 1).toString().padStart(2, '0')}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`;
      
      console.log('General POA Application:', { ...formData, referenceNumber });
      setIsSubmitting(false);
      
      navigate('/success', { 
        state: { 
          referenceNumber, 
          serviceTitle: generalConfig.title 
        } 
      });
    }, 2000);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1 text-center px-4">
              <h1 className="text-2xl sm:text-3xl font-bold">{generalConfig.title}</h1>
            </div>
